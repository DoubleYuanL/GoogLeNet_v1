...
def forward_propagation(X,keep_prob):
    ...
    Z1 = tf.contrib.layers.conv2d(inputs=X, num_outputs=64, kernel_size=[7,7], stride=[2,2], padding='SAME', activation_fn=tf.nn.relu) 
    ...
    A1 = tf.contrib.layers.avg_pool2d(inputs=I3, kernel_size=[5,5], stride=[2,2], padding='SAME')
    I3 = tf.contrib.layers.conv2d(inputs=A1, num_outputs=16, kernel_size=[1,1], stride=[1,1], padding="SAME", activation_fn=None)
    Fa1 = tf.contrib.layers.flatten(I3)
    Z9 = tf.contrib.layers.fully_connected(Fa1,64,activation_fn=None)
    ...
    A2 = tf.contrib.layers.avg_pool2d(inputs=I7, kernel_size=[5,5], stride=[2,2], padding='SAME')
    Z15 = tf.contrib.layers.conv2d(inputs=A2, num_outputs=16, kernel_size=[1,1], stride=[1,1], padding="SAME", activation_fn=None)
    Fa2 = tf.contrib.layers.flatten(Z15)
    Z15 = tf.contrib.layers.fully_connected(Fa2,64,activation_fn=None)
    Z15 = tf.contrib.layers.fully_connected(Z15,6,activation_fn=None)
    print("Z15"+str(Z15.shape))

    I8 = inception_module_v1(I7)#inception (5a)
    I9 = inception_module_v1(I8)#inception (5b)
    print("I9"+str(I9.shape))
    A3 = tf.contrib.layers.avg_pool2d(inputs=I9, kernel_size=[7,7], stride=[2,2], padding='SAME')
    Fa3 = tf.contrib.layers.flatten(A3)
    Z22 = tf.contrib.layers.fully_connected(Fa3,6,activation_fn=None)#tf.nn.softmax)
    print("Z22"+str(Z22.shape))
